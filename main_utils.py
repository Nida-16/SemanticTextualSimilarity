...
def to_df_input_data(text1, text2):
    text1 = [text1]
    text2 = [text2]
    input_df = pd.DataFrame(list(zip(text1, text2)),
                            columns=['text1', 'text2'])
    return input_df
...
def preprocess_data(data_frame):
    ...
    for i in range(len(data['text1'])):
        t1 = data['text1'][i]
        t2 = data['text2'][i]
        data['text1'][i] = t1.replace('/', ' ').replace('.', ' ').replace(
            '-', ' ').replace('!', ' ').replace('%', ' ').replace(',', ' ')
        data['text2'][i] = t2.replace('/', ' ').replace('.', ' ').replace(
            '-', ' ').replace('!', ' ').replace('%', ' ').replace(',', ' ')
    ...
    # Removing stop words
    for i in range(len(data['text1'])):
        data['text1'][i] = [token for token in data['text1']
                            [i] if token not in stop_words]
        data['text2'][i] = [token for token in data['text2']
                            [i] if token not in stop_words]

    # Removing punctuations and Lemmatizing words
    for i in range(len(data['text1'])):
        data['text1'][i] = [
            lemmatize(token) for token in data['text1'][i] if token not in punctuations]
        data['text2'][i] = [
            lemmatize(token) for token in data['text2'][i] if token not in punctuations]

    # Removing Numbers
    data['text1'] = data['text1'].apply(
        lambda x: [token for token in x if not token.isdigit()])
    data['text2'] = data['text2'].apply(
        lambda x: [token for token in x if not token.isdigit()])

    # Filtering out empty string tokens
    data['text1'] = data['text1'].apply(
        lambda x: [token for token in x if token != ''])
    data['text2'] = data['text2'].apply(
        lambda x: [token for token in x if token != ''])
...
#     Calculating similarity of texts
    ip_similarities = similarity_calculation_function_for_input(
        prepped_input_df, ip_similarities)
